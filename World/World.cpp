...
void World::RenderWorld(GLFWwindow* EngineWindow,Camera *EngineCamera,float Near,float Far,float FieldOfView,unsigned int ReflectanceTexture,float DeltaTime,float PlayerSpeed)
{
    glEnable(GL_CULL_FACE);
    glEnable(GL_DEPTH_TEST);
    this->NewInstance.RenderEntities(EngineCamera,Near,Far,FieldOfView,EngineCamera->CameraMatrix);
    this->WeaponViewport.RenderViewport(EngineWindow,EngineCamera,Near,Far,FieldOfView,PlayerSpeed);
}
